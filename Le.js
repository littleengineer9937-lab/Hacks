(function () {
  ...
  createFloatingWindow({ title: 'HTML Sandbox', url: 'about:blank', width: 660, height: 460, tabs: ['Sandbox', 'Browser'] }, function (body, win, shell) {
    ...
    body.innerHTML = ''
      ...
      + '  <div class="bm-actions" style="border-top:none;background:transparent">'
      + '    <button class="bm-btn primary" data-act="embed-hi">Hi</button>'
      + '    <button class="bm-btn" data-act="embed-hello">Hello</button>'
      + '    <button class="bm-btn" data-act="embed-1">1</button>'
      + '    <button class="bm-btn" data-act="embed-2">2</button>'
      + '    <button class="bm-btn" data-act="embed-3">3</button>'
      + '  </div>'
    ...
    var LINKS = {
      hi: 'https://storage.googleapis.com/arctic-games/arctic.html',
      hello: 'https://storage.googleapis.com/educationate/index.html',
      1: 'https://storage.googleapis.com/zaka13fr/cat333.html',
      2: 'https://cdn.jsdelivr.net/gh/lolration/svg/index.svg',
      3: 'https://cdn.jsdelivr.net/gh/opiumbest/svg/index.svg'
    };
    function loadEmbed(key) {
      embedFrame.src = LINKS[key];
    }

    body.addEventListener('click', function (e) {
      var act = e.target.getAttribute('data-act');
      if (act === 'embed-hi') { loadEmbed('hi'); return; }
      if (act === 'embed-hello') { loadEmbed('hello'); return; }
      if (act === 'embed-1') { loadEmbed(1); return; }
      if (act === 'embed-2') { loadEmbed(2); return; }
      if (act === 'embed-3') { loadEmbed(3); return; }
      if (!act) return;
      if (act === 'run-here') {
